                            ...
                            @if ($language)
                                <div class="lang_form" id="default-form">
                                    ...
                                    <div class="form-group pt-2 mb-0">
                                        ...
                                        <textarea name="description[]" class="form-control ckeditor min--height-200">{!! $product?->getRawOriginal('description') !!}</textarea>
                                    </div>
                                </div>
                                @foreach (json_decode($language) as $lang)
                                    <?php
...
<script>
  ...
  $('#product_form').on('submit', function(e) {
    ...
    // Wait a short time to ensure CKEditor updates are applied
    setTimeout(() => {
        ...
        let formData = new FormData($('#product_form')[0]); // Use FormData
        ...
        $.ajax({
            url: $('.route_url').val(), // Get URL dynamically
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
    ...
    }, 100); // Delay to allow CKEditor updates
});
