...
@section('content')
    ...
    <div class="content container-fluid">
        ...
        @endphp
        @php
    // Split the schedule_at into start and end times
    $start_time = null;
    $end_time = null;
    if ($order['schedule_at'] && !empty($order['schedule_at'])) {
        if (strpos($order['schedule_at'], ' to ') !== false) {
            $schedule_parts = explode(' to ', $order['schedule_at']);
            if (isset($schedule_parts[0]) && !empty(trim($schedule_parts[0]))) {
                try {
                    $start_time = \Carbon\Carbon::parse(trim($schedule_parts[0]));
                } catch (\Exception $e) {
                    $start_time = null;
                }
            }
            if (isset($schedule_parts[1]) && !empty(trim($schedule_parts[1]))) {
                try {
                    $end_time = \Carbon\Carbon::parse(trim($schedule_parts[1]));
                } catch (\Exception $e) {
                    $end_time = null;
                }
            }
            ...
            try {
                $start_time = \Carbon\Carbon::parse($order['schedule_at']);
            } catch (\Exception $e) {
                $start_time = null;
            }
        }
                            ...
                            <div>
                                ...
                                @if ($order->schedule_at && $order->scheduled)
                                    <h6 class="text-capitalize d-flex align-items-center __gap-5px">
                                        ...
                                        <span>:</span> <label class="fz--10 badge badge-soft-warning">
                                        @if ($start_time && $end_time)
                                            {{ $start_time->format('d M Y ' . config('timeformat')) }} to {{ $end_time->format('d M Y ' . config('timeformat')) }}
                                        @else
                                            {{ $order['schedule_at'] }}  {{-- Fallback if no valid start and end time found --}}
                                        @endif
                                    </label>
                                    </h6>
                                @endif
                                                        ...
                                                        <div class="media media--sm">
                                                            ...
                                                            <div class="media-body">
                                                                <div>
                                                                    ...
                                                                    @if ($order->store && $order->store->module->module_type == 'food')
                                                                        @php($variationDecoded = isset($detail['variation']) ? json_decode($detail['variation'], true) : [])
                                                                        @if ($variationDecoded)
                                                                            @foreach ($variationDecoded as $variation)
                                                                                @if (is_array($variation) && isset($variation['name']) && isset($variation['values']) && is_array($variation['values']))
                                                                                    ...
                                                                                    @endforeach
                                                                                @else
                                                                                    @if (is_array($variationDecoded) && isset($variationDecoded[0]) && is_array($variationDecoded[0]))
                                                                                        <strong><u>
                                                                                                {{ translate('messages.Variation') }}
                                                                                                : </u></strong>
                                                                                        @foreach ($variationDecoded[0] as $key1 => $variation)
                                                                                            <div
                                                                        ...
                                                                        @endif
                                                                    @else
                                                                        @php($variationDecoded = json_decode($detail['variation'], true))
                                                                        @if (is_array($variationDecoded) && isset($variationDecoded[0]) && is_array($variationDecoded[0]) && count($variationDecoded) > 0)
                    ...
                    <div class="card-body pt-3">
                        @if ($order->customer && $order->is_guest == 0)
                            ...
                            @if ($order->receiver_details)
                                @php($receiver_details = is_array($order->receiver_details) ? $order->receiver_details : (json_decode($order->receiver_details, true) ?? []))
                                ...
                                @if (isset($receiver_details))
                                    <span class="delivery--information-single mt-3">
                                        ...
                                        <span class="info">{{ data_get($receiver_details,'contact_person_name', translate('messages.N/A')) }}</span>
                                        <span class="name">{{ translate('messages.contact') }}</span>
                                        <a class="deco-none info d-flex"
                                            href="tel:{{ data_get($receiver_details,'contact_person_number','') }}">
                                            {{ data_get($receiver_details,'contact_person_number', translate('messages.N/A')) }}</a>
            ...
            function initializegLocationMap() {
                ...
                @if ($parcel_order && isset($receiver_details))
                    var Receivermarker = new google.maps.Marker({
                        ...
                        map: map,
                        title: "{{ Str::limit(data_get($receiver_details,'contact_person_name',''), 15, '...') }}",
                        // icon: "{{ asset('assets/admin/img/restaurant_map.png') }}"
                    });
