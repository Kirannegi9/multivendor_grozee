...
{{--                    {{dd($cartItem)}}--}}
    ...
    <?php
        ...
        $surge_charge_status = \App\Models\BusinessSetting::where('key', 'surge_charge_status')->first()->value;
        $surge_charge = \App\Models\BusinessSetting::where('key', 'surge_charge')->first()->value;
        if ($surge_charge_status == 1) {
            $surge_charges = $surge_charge ?? 0;
        } else {
          $surge_charges = 0;
        }
        $processing_charge_status = \App\Models\BusinessSetting::where('key', 'processing_charge_status')->first()->value;
        $processing_charge = \App\Models\BusinessSetting::where('key', 'processing_charge')->first()->value;
        if ($processing_charge_status == 1) {
            $processing_charges = $processing_charge ?? 0;
        } else {
            $processing_charges = 0;
        }
        
        $smallcart_charge_status = \App\Models\BusinessSetting::where('key', 'smallcart_charge_status')->first()->value;
        $smallcart_charge = \App\Models\BusinessSetting::where('key', 'smallcart_charge')->first()->value;
        $smallcart_value = \App\Models\BusinessSetting::where('key', 'smallcart_value')->first()->value;

        if ($smallcart_charge_status == 1 && $total_tax_amount+$total < $smallcart_value) {
          $smallcart_charges = $smallcart_charge ?? 0;
        } else {
            $smallcart_charges = 0;
        }
        
        $grand_total = ($total + $total_tax_amount + ($surge_charges ?? 0) + ($processing_charges ?? 0) + ($smallcart_charges ?? 0));
    ...
    <div class="box p-3">
        <dl class="row text-dark">
            ...
            @endif
            <dd class="col-6">{{ translate('messages.Small cart Fee') }}:</dd>
                                        <dd class="col-6 text-right">
                                            + {{ \App\CentralLogics\Helpers::format_currency($smallcart_charges ?? 0) }}

                                        </dd>
                                        
            <dd class="col-6">{{ translate('messages.Surge Fee') }}:</dd>
                                        <dd class="col-6 text-right">
                                            + {{ \App\CentralLogics\Helpers::format_currency($surge_charges ?? 0) }}
                                        </dd>
                                        <dd class="col-6">{{ translate('messages.Processing Fee') }}:</dd>
                                        <dd class="col-6 text-right">
                                            + {{ \App\CentralLogics\Helpers::format_currency($processing_charges ?? 0) }}
                                            <hr>
                                        </dd>
            <dd  class="col-6 pr-0">
                <hr class="my-0">
            ...
            <dt class="col-6 text-right">
                {{\App\CentralLogics\Helpers::format_currency(round($grand_total, 2))}}
            </dt>
        </dl>
