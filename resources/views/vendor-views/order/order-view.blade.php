                        ...
                        <div class="row justify-content-md-end mb-3 mx-0 mt-4">
                            <div class="col-md-9 col-lg-8">
                                <dl class="row text-right">
                                    ...
                                    <dd class="col-6">
                                        + {{ \App\CentralLogics\Helpers::format_currency($order->smallcart_charge ?? 0) }}
                                    </dd>
                                    <dt class="col-6">{{ translate('messages.Surge   Fee') }}:</dt>
                                    <dd class="col-6">
                                        + {{ \App\CentralLogics\Helpers::format_currency($order->surge_charge ?? 0) }}
                                    </dd>
                                    <dt class="col-6">{{ translate('messages.Processing Fee') }}:</dt>
                                    <dd class="col-6">
                                        + {{ \App\CentralLogics\Helpers::format_currency($order->processing_charge ?? 0) }}
                                        <hr>
                                    </dd>
                                    ...
                                    <dd class="col-6">
                                        {{ \App\CentralLogics\Helpers::format_currency($product_price + $del_c + $total_tax_amount + $total_addon_price + $additional_charge - $coupon_discount_amount - $store_discount_amount - $admin_flash_discount_amount  - $ref_bonus_amount + $extra_packaging_amount-$store_flash_discount_amount + $order->dm_tips + ($order->surge_charge ?? 0) + ($order->smallcart_charge ?? 0) + ($order->processing_charge ?? 0)) }}
                                    </dd>
                                    @if ($order?->payments)
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('vendor.order.update-order-amount') }}" method="POST" class="row">
                        ...
                        <div class="form-group col-12">
                            <label for="surge_charge">{{ translate('messages.Surge Fee') }}</label>
                            <input id="surge_charge" type="number" class="form-control" name="surge_charge" min="0" step=".01"
                                   value="{{ round($order->surge_charge ?? 0, 2) }}">
                        </div>
                        <div class="form-group col-12">
                            <label for="processing_charge">{{ translate('messages.Processing Fee') }}</label>
                            <input id="processing_charge" type="number" class="form-control" name="processing_charge" min="0" step=".01"
                                   value="{{ round($order->processing_charge ?? 0, 2) }}">
                        </div>
                        <div class="form-group col-12">
                            <label for="smallcart_charge">{{ translate('messages.Small cart Fee') }}</label>
                            <input id="smallcart_charge" type="number" class="form-control" name="smallcart_charge" min="0" step=".01"
                                   value="{{ round($order->smallcart_charge ?? 0, 2) }}">
                        </div>
