                        ...
                        <div class="card-body">
                            ...
                            id="default-form">
                                ...
                                <div class="form-group mb-0">
                                    ...
                                    <textarea name="description[]" class="form-control min-h-90px ckeditor"></textarea>
                                </div>
                            </div>
                                @foreach (json_decode($language) as $lang)
...
$('#item_form').on('submit', function(e) {
    ...
    // Ensure CKEditor updates the textarea value before submission
    for (let instanceName in CKEDITOR.instances) {
        CKEDITOR.instances[instanceName].updateElement();
    }

    // Wait a short time to ensure the update is applied
    setTimeout(() => {
        let descriptionValue = $('textarea[name="description[]"]').val().trim();
        console.log("Final Description Value:", descriptionValue);

        if (descriptionValue === '') {
            alert("Description field is still empty!");
            return;
        }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.errors) {
                    ...
                    toastr.success("{{ translate('messages.product_added_successfully') }}", { CloseButton: true, ProgressBar: true });
                    setTimeout(() => location.href = "{{ route('admin.item.list') }}", 1000);
                }
            }
        });

    }, 100); // Delay to allow CKEditor to update the textarea
});
