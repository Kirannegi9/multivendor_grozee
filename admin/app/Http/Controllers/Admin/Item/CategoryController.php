...
class CategoryController extends BaseController
{
    ...
    private function getCategoryView(Request $request): View
    {
        $categories = $this->categoryRepo->getListWhere(
            ...
            relations: ['module','headCategory'],
            dataLimit: config('default_pagination')
        );
        $headCategories  = DB::table('head_category')->get();
        ...
        return view($this->categoryService->getViewByPosition($request['position']), compact('categories','language','defaultLang','mainCategories','headCategories'));
    }
    ...
    public function getUpdateView(string|int $id): View
    {
        ...
        return view(CategoryViewPath::UPDATE['view'], compact('category','language','defaultLang','headCategories'));
    }
...
public function head_category_store(Request $request)
{
    $request->validate([
        'name' => 'required|string|max:191|unique:categories,name',
    ]);

    DB::table('head_category')->insert([
        'name' => $request->name,
        'created_at' => now(),
        'updated_at' => now(),
    ]);

    return redirect()->route('admin.head-category.indexx')->with('success', 'Head Category added successfully.');
}

public function head_category_edit($id)
{
    $category = DB::table('head_category')->where('id', $id)->first();
    return view('admin-views.category.edit-head-category', compact('category'));
}

public function head_category_update(Request $request, $id)
{
    $request->validate([
        'name' => 'required|string|max:191|unique:head_category,name,' . $id,
    ]);

    DB::table('head_category')->where('id', $id)->update([
        'name' => $request->name,
        'updated_at' => now(),
    ]);

    return redirect()->route('admin.head-category.indexx')->with('success', 'Head Category updated successfully.');
}
public function head_category_delete($id)
{
    DB::table('head_category')->where('id', $id)->delete();
    return redirect()->route('admin.head-category.indexx')->with('success', 'Head Category deleted successfully.');
}
