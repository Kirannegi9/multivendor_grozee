...
class ConfigController extends Controller
{
    ...
    public function configuration()
    {
        ...
        $surge_charge = isset($settings['surge_charge'])?(float)$settings['surge_charge']:0;
        $processing_charge = isset($settings['processing_charge'])?(float)$settings['processing_charge']:0;
        $smallcart_charge = isset($settings['smallcart_charge'])?(float)$settings['smallcart_charge']:0;
        $module = null;
...
//            'landing_page_settings_full_url'=> $data['web_app_landing_page_settings_storage']??'public',
            ...
            'surge_charge_status' => (int)(isset($settings['surge_charge_status']) ? $settings['surge_charge_status'] : 0),
            'surge_charge'=>$surge_charge,
            'processing_charge_status' => (int)(isset($settings['processing_charge_status']) ? $settings['processing_charge_status'] : 0),
            'processing_charge'=>$processing_charge,
            'smallcart_charge_status' => (int)(isset($settings['smallcart_charge_status']) ? $settings['smallcart_charge_status'] : 0),
            'smallcart_value' => (isset($settings['smallcart_value']) ? $settings['smallcart_value'] : 0),
            'smallcart_charge'=>$smallcart_charge,
 ...
 public function get_zone(Request $request)
{
    ...
    try {
        // Fetch zone IDs that contain the given coordinates
        $zoneIds = DB::select("
            SELECT id 
            FROM zones 
            WHERE ST_CONTAINS(coordinates, ST_GeomFromText(?, 4326)) 
        ...
        if (empty($zoneIds)) {
            return response()->json([
                'errors' => [['code' => 'coordinates', 'message' => translate('messages.service_not_available_in_this_area')]]
            ], 404);
        }

        $zoneIds = array_column($zoneIds, 'id');

        // Fetch full zone details, including modules & translations
        $zones = Zone::with(['modules', 'translations'])
            ->whereIn('id', $zoneIds)
            ->get();

        // Filter only active zones
        $activeZones = $zones->where('status', 1)->values();

        if ($activeZones->isNotEmpty()) {
            return response()->json([
                'zone_id'   => json_encode($zoneIds),  // Keep the "[5]" format
                'zone_data' => $activeZones
            ], 200);
        }

        return response()->json([
            'errors' => [['code' => 'coordinates', 'message' => translate('messages.we_are_temporarily_unavailable_in_this_area')]]
        ], 403);
