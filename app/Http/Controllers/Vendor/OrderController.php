...
//    }
    ...
    public function edit_order_amount(Request $request)
    {
        $request->validate([
            ...
            'surge_charge' => 'nullable|numeric|min:0',
            'processing_charge' => 'nullable|numeric|min:0',
            'smallcart_charge' => 'nullable|numeric|min:0',
        ]);
        ...
        if($request->has('surge_charge')){
            $order->surge_charge = round($request->surge_charge, config('round_up_to_digit'));
        }
        if($request->has('processing_charge')){
            $order->processing_charge = round($request->processing_charge, config('round_up_to_digit'));
        }
        if($request->has('smallcart_charge')){
            $order->smallcart_charge = round($request->smallcart_charge, config('round_up_to_digit'));
        }
        $order->order_amount = round($total_price + $tax_a + $order->additional_charge + $order->delivery_charge, config('round_up_to_digit'));
        $order->order_amount = $order->order_amount + ($order->surge_charge ?? 0) + ($order->smallcart_charge ?? 0) + ($order->processing_charge ?? 0) + ($order->extra_packaging_amount ?? 0);
    ...
    public function edit_discount_amount(Request $request)
    {
        ...
        $order->order_amount = $product_price+$order['delivery_charge']+ $tax_a +$order['dm_tips'] + $order->additional_charge  -$order->store_discount_amount + ($order->surge_charge ?? 0) + ($order->smallcart_charge ?? 0) + ($order->processing_charge ?? 0) + ($order->extra_packaging_amount ?? 0);
        $order->save();
