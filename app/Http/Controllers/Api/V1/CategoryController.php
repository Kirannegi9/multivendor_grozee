...
class CategoryController extends Controller
{
    ...
    public function get_head_categories(Request $request)
{
    try {
        ...
        // Group categories under head categories
        $headCategoryData = $headCategories->map(function ($headCategory) use ($categories, $zone_id, $category_list_default_status, $category_list_sort_by_general) {
            $filteredCategories = $categories->where('head_category_id', $headCategory->id)->values();
            ...
            if (count($zone_id) > 0) {
                ...
                if ($category_list_default_status != 1 && $category_list_sort_by_general == 'order_count') {
                    $filteredCategories = $filteredCategories->sortByDesc('order_count')->values();
                }
            }
        ...
        return response()->json($headCategoryData, 200);
    } catch (\Exception $e) {
