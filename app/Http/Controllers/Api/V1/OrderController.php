...
class OrderController extends Controller
{
    ...
    public function place_order(Request $request)
    {
        ...
        $validator->sometimes('otp', 'required', function ($request) {
            return in_array($request['payment_method'], ['digital_payment', 'wallet']) || ($request['payment_method'] == 'cash_on_delivery' && ($request['order_type'] ?? '') != 'take_away');
        });
        ...
        $requiresOtp = in_array($request->payment_method, ['digital_payment', 'wallet']) || ($request->payment_method == 'cash_on_delivery' && ($request->order_type ?? '') != 'take_away');
        if ($requiresOtp) {
            $phone = $request->user ? $request->user->phone : ($request->contact_person_number ? str_replace('+', '', $request->contact_person_number) : null);
            if (!$phone) {
                return response()->json(['errors' => [ ['code' => 'otp', 'message' => translate('messages.not_found')] ]], 403);
            }
            $data = DB::table('phone_verifications')->where(['phone' => $phone, 'token' => $request->otp])->first();
            // if (!$data) {
            //     return response()->json(['errors' => [ ['code' => 'otp', 'message' => translate('messages.phone_number_and_otp_not_matched')] ]], 403);
            // }
            DB::table('phone_verifications')->where(['phone' => $phone, 'token' => $request->otp])->delete();
        }
...
$schedule_start = $schedule_end = null;
$schedule_at = null;

if ($request->filled('schedule_at') && str_contains($request->schedule_at, ' to ')) {
    // Split start and end time
    [$start, $end] = explode(' to ', $request->schedule_at);

    try {
        // Parse both start and end times as Carbon instances
        $schedule_start = Carbon::parse(trim($start));
        $schedule_end = Carbon::parse(trim($end));

        // Combine into a single string for storage (only here we format)
        $schedule_at = $schedule_start->format('Y-m-d H:i') . ' to ' . $schedule_end->format('Y-m-d H:i');
    } catch (\Exception $e) {
        // Handle invalid format and log the error
        \Log::error('Invalid schedule_at format', ['error' => $e->getMessage(), 'input' => $request->schedule_at]);

        // Set default fallback (no formatting, just Carbon instances)
        $schedule_start = now();
        $schedule_end = now()->addHour();
        // Don't format here; just store as Carbon instances
        $schedule_at = $schedule_start . ' to ' . $schedule_end; // Optional: combine as string without format
    }
} else {
    // No `schedule_at` provided — fallback to default range (as Carbon instances)
    $schedule_start = now();
    $schedule_end = now()->addHour();
    // Don't format here; just store as Carbon instances
    $schedule_at = $schedule_start . ' to ' . $schedule_end; // Optional: combine as string without format
    ...
    $digital_payment = Helpers::get_business_settings('digital_payment');
       ...
       if ($request->latitude && $request->longitude) {
        ...
        // ✅ Validate & format latitude & longitude
        $longitude = is_array($request->longitude) ? trim($request->longitude[0], '"') : trim($request->longitude, '"');
        $latitude = is_array($request->latitude) ? trim($request->latitude[0], '"') : trim($request->latitude, '"');

        if (!is_numeric($longitude) || !is_numeric($latitude)) {
            throw new \Exception("Invalid coordinates. Expected numeric values.");
        }
        ...
        if ($request->order_type == 'parcel') {
            ...
            $zone = Zone::whereIn('id', $zone_ids)
                ->whereRaw("ST_CONTAINS(coordinates, ST_GeomFromText('POINT($longitude $latitude)', 4326))") // ✅ Correct SQL
            ...
            $store_id = is_array($request->store_id) ? trim($request->store_id[0], '"') : trim($request->store_id, '"');
            ...
            $store = Store::with(['discount', 'store_sub'])
                ->selectRaw('*, IF(((SELECT COUNT(*) FROM `store_schedule` 
                    ...
                    // [$schedule_at->format('w'), $schedule_at->format('H:i:s'), $schedule_at->format('H:i:s')]
                    [
            $schedule_start->format('w'), // Day of the week
            $schedule_start->format('H:i:s'), // Start time in H:i:s format
            $schedule_end->format('H:i:s') // End time in H:i:s format
        ]
                )
                ->where('id', $store_id)
                ->first();
            ...
            if (!$store) {
                return response()->json([
                    'errors' => [['code' => 'order_time', 'message' => translate('messages.store_not_found')]]
                ], 404);
            }
            ...
            $zone = Zone::whereIn('id', $zone_id)
                ->whereRaw("ST_CONTAINS(coordinates, ST_GeomFromText('POINT($longitude $latitude)', 4326))") // ✅ Fix Geospatial Query
                ->first();
        }
 ...
 Log::info('scheduled @ 456', [
        'scheduled' => $request->scheduled,
    ]);
        if ($request->order_type !== 'parcel') {
            // if ($request->schedule_at && $schedule_at < now()) {
            //     Log::info('schedule_at@123', ['schedule_at' => $request->schedule_at]);
            //     return response()->json([
            //         'errors' => [
            ...
            //     ], 406);
            // }
if ($request->schedule_at && str_contains($request->schedule_at, ' to ')) {
    [$start, $end] = explode(' to ', $request->schedule_at);
    $startTime = Carbon::parse(trim($start));
    $endTime = Carbon::parse(trim($end));
    $now = now();

    Log::info('schedule_at@123', [
    ...
    // Reject only if the entire schedule is in the past
    if ($endTime->lt($now)) {
        return response()->json([
            'errors' => [
                ['code' => 'order_time', 'message' => translate('messages.you_can_not_schedule_a_order_in_past')]
      ...
      Log::info('schedule_at', ['schedule_at' => $request->schedule_at]);


            if ($store->open == false && !$request->schedule_at) {
                return response()->json([
                    'errors' => [
...
//            'contact_person_number' => $request->contact_person_number ? ($request->user ? $request->contact_person_number :str_replace('+', '', $request->contact_person_number)) : ($request->user?$request->user->phone:''),
        ...
        $order->receiver_details = is_array($request->receiver_details)
        ? json_encode($request->receiver_details)
        : $request->receiver_details;
        ...
        $surge_charge_status = BusinessSetting::where('key', 'surge_charge_status')->first()->value;
        $surge_charge = BusinessSetting::where('key', 'surge_charge')->first()->value;
        if ($surge_charge_status == 1) {
            $order->surge_charge = $surge_charge ?? 0;
        } else {
            $order->surge_charge = 0;
        }
        $processing_charge_status = BusinessSetting::where('key', 'processing_charge_status')->first()->value;
        $processing_charge = BusinessSetting::where('key', 'processing_charge')->first()->value;
        if ($processing_charge_status == 1) {
            $order->processing_charge = $processing_charge ?? 0;
        } else {
...
if ($smallcart_charge_status == 1 && $request['order_amount'] < $smallcart_value) {
    ...
    $order->smallcart_charge = 0;
        ...
        //DM TIPS
        $order->order_amount = $order->order_amount + $order->dm_tips + $order->additional_charge + $order->surge_charge +  $order->smallcart_charge + $order->processing_charge + $order->extra_packaging_amount;
...
//   if ($request->payment_method == 'cash_on_delivery' && !$request->partial_payment){
//         $initial_payment = $request['order_amount'] * 0.10;
...
//     }
        ...
        try {
            ...
            DB::beginTransaction();
//               $order->partially_paid_amount = $initial_payment;
//               $order->payment_status = "partially_paid";
//               $order->payment_method = "partial_payment";
//                  Log::info('Initial 10% COD payment calculated123:', [
//     'order_amount' => $order->partially_paid_amount,
// ]);
            $order->save();
            if ($request->order_type !== 'parcel') {
