  ...
  public static function razorpay()
    {
        ...
        return $service->setRazorpayConfig();
    }
    ...
    public static function address_data_formatting($data)
{
    foreach ($data as $key => $item) {
        ...
        // Fix: Use raw query for ST_CONTAINS check
        $zoneIds = DB::select("
            SELECT id FROM zones 
            WHERE ST_CONTAINS(coordinates, ST_GeomFromText(?, 4326))
            ORDER BY created_at DESC", ["POINT($longitude $latitude)"]);

        $data[$key]['zone_ids'] = array_column($zoneIds, 'id');
    }
    ...
    public static function order_details_data_formatting($data)
{
    ...
    foreach ($data as $item) {
        ...
        // Ensure item_details is not null and contains an id
        if (!empty($item['item_details']) && isset($item['item_details']['id'])) {
            if ($item['item_id']) {
                $product = \App\Models\Item::where(['id' => $item['item_details']['id']])->first();
            } else {
                $product = \App\Models\ItemCampaign::where(['id' => $item['item_details']['id']])->first();
            ...
            $item['image_full_url'] = $product ? $product->image_full_url : null;
            $item['images_full_url'] = $product ? $product->images_full_url : [];
        } else {
    ...
    public static function send_order_notification($order)
    {
        ...
        try {
            ...
            if (in_array($order->order_status, ['processing', 'handover']) && $order->delivery_man && self::getNotificationStatusData('deliveryman','deliveryman_order_notification','push_notification_status')) {
                $data = [
                    'title' => translate('Order_Notification'),
                    'description' => $order->order_status == 'processing' ? translate('Your order is processing') : translate('messages.ready_for_delivery'),
                    'order_id' => $order->id,
                    'image' => '',
                    'type' => 'order_status'
                ];
